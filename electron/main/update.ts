...
export async function autoUpdateApp() {
  ...
  autoUpdater.on('update-available', (info) => {
    ...
    const { version } = info
    logger.info(`最新版本为： ${version}`) // 这里做的是检测到更新，直接就下载
  ...
  autoUpdater.on('update-downloaded', (res) => {
    logger.info('下载完毕, 提示安装更新', res) // 这里需要注意，Electron.dialog 想要使用，必须在 BrowserWindow 创建之后
    dialog
      .showMessageBox({
        title: '更新应用',
      ...
      .then(() => {
        logger.info('退出应用，安装开始！') // 重启应用并在下载后安装更新，它只应在发出 update-downloaded 这个事件后方可被调用。
